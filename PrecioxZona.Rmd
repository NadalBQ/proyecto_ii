...
```{r}
summary(lm(dollarPrice ~ bedrooms + bathrooms + accommodates, data = pisos_filtrados))
```
...
kmeans_result <- kmeans(data_clean %>% select(peso), centers = 2)
...
# Realizar el clustering con k=2
kmeans_result <- kmeans(zonas_clean %>% select(relacion_precio_media), centers = 2)
...
pal <- colorFactor(c("red", "blue"), domain = c(1, 2))  # Ajusta los colores según tus clusters
