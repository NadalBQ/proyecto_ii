...
```{r}
mySD = apply(list_limpiado_pisos2[,descValList$variable[descdat$tipo == "numerical"]], 2, sd, na.rm=TRUE)   #Calculamos la desviación típica (SD) 
mySD
```
# Mejor calcular el coeficiente de variación porque no depende de las unidades o magnitud de las variables
```{r}
myMU = colMeans(list_limpiado_pisos2[,descValList$variable[descdat$tipo == "numerical"]], na.rm=TRUE)
myMU#La media
...
binarias <- which(sapply(list_limpiado_pisos2, function(x) length(unique(x)) == 2))

# Crear un nuevo dataframe excluyendo las columnas binarias
list_sin_binarias <- list_limpiado_pisos2[, -binarias]
```
