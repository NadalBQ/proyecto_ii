...
## K-medias
```{r}
p1 = fviz_nbclust(x = pisos_completo, FUNcluster = kmeans, method = "silhouette", 
             k.max = 10, verbose = FALSE) +
  labs(title = "K-means")
p2 = fviz_nbclust(x = pisos_completo, FUNcluster = kmeans, method = "wss", 
             k.max = 10, verbose = FALSE) +
  labs(title = "K-means")
grid.arrange(p1, p2, nrow = 1)
```
...
```{r}
set.seed(100)
clust3 <- kmeans(pisos_completo, centers = 4, nstart = 20)
table(clust3$cluster)
```
## K-medoides
```{r}
p1 = fviz_nbclust(x = pisos_completo, FUNcluster = pam, method = "silhouette", 
             k.max = 10, verbose = FALSE) +
  labs(title = "Numero optimo de clusters")
p2 = fviz_nbclust(x = pisos_completo, FUNcluster = pam, method = "wss", 
             k.max = 10, verbose = FALSE) +
  labs(title = "Numero optimo de clusters")
grid.arrange(p1, p2, nrow = 1)
```
...
validacion = suppressMessages(clValid(pisos_completo, nClust = 4:5, metric = "euclidean", 
                      clMethods = metodos, 
...
```{r}
set.seed(100)
clustprueba <- kmeans(pisos_completo, centers = 4, nstart = 20)
table(clustprueba$cluster)
```

## Interpretación

```{r}
misclust = factor(clustprueba$cluster)
miPCA = PCA(pisos_completo, scale.unit = FALSE, graph = FALSE)
...
p1 = fviz_pca_ind(miPCA, geom = "point", habillage = misclust, 
                  addEllipses = TRUE, axes = c(1,3), palette = colores)
p2 = fviz_pca_var(miPCA, axes = c(1,3))
grid.arrange(p1, p2, nrow = 1)
p2
```
1 componente: transportre
2: distancias
3: vivienda (baños, accomodates, bedrooms)

```{r}
mediasCluster = aggregate(pisos_completo, by = list("cluster" = misclust), mean)[,-1]
...
axis(side = 1, at = 1:ncol(pisos_completo), labels = colnames(pisos_completo), las = 2)
legend("topleft", as.character(1:4), col = colores, lwd = 2, ncol = 3, bty = "n")
```
```{r}
boxplot(pisos_completo_precio$precio_ponderado ~ misclust, col = colores)
```
